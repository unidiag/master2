...
if ($carryDebt) {
     ...
     *
     * 1. Сейчас есть долг.
     * 2. За историю была хотя бы одна оплата:
     *      summ стало меньше предыдущего summ.
     * 3. Ни одна оплата не погашала задолженность
     *    полностью, т.е. после оплаты summ всегда > 0.
     */
    ...
    $where .= '
        ...
        AND current_db.personal IN (
            SELECT payment_history.personal
            FROM (
                SELECT
            ...
            GROUP BY
                payment_history.personal

            HAVING
                /*
                 * Была хотя бы одна оплата.
                 */
                SUM(
                    CASE
                        WHEN
                            payment_history.previous_summ
                                IS NOT NULL

                            AND payment_history.summ_value
                                <
                                payment_history.previous_summ
                ...
                /*
                 * Ни одна оплата не закрывала долг
                 * полностью.
                 */
                SUM(
                    CASE
...
public function debtors(): array
{
    ...
    $statement = $this->db->prepare(
        'WITH history AS (
            SELECT
                ...
                personal,
                CAST(`update` AS UNSIGNED) AS update_ts,
                CAST(summ AS DECIMAL(15, 2)) AS summ_value,
                LAG(
                    CAST(summ AS DECIMAL(15, 2))
                ) OVER (
                    PARTITION BY personal
                    ORDER BY
                        CAST(`update` AS UNSIGNED),
                        id
                ) AS previous_summ
        ...
        last_payments AS (
            SELECT
                personal,
                MAX(update_ts) AS last_payment_update
            FROM history
            WHERE previous_summ IS NOT NULL
            AND summ_value < previous_summ
            GROUP BY personal
        )
        SELECT
            db.id,
            ...
            db.tarif,
            db.`update`,
            lp.last_payment_update,
            COALESCE(k.descr, \'\') AS karandash_descr
        FROM master_database AS db
        LEFT JOIN master_karandash AS k
            ON k.address = db.address
        LEFT JOIN last_payments AS lp
            ON lp.personal = db.personal
        WHERE db.`update` = :update
        AND db.tarif <> :no_contract
        AND CAST(db.summ AS DECIMAL(15, 2)) > 0
        AND (
            lp.last_payment_update IS NULL
            OR lp.last_payment_update <
                UNIX_TIMESTAMP(
    ...
    while ($row = $statement->fetch(PDO::FETCH_ASSOC)) {
        ...
        $houses[$houseName]['items'][] = [
            ...
            'debt' => $sum,
            'last_payment_update' => trim(
                (string) (
                    $row['last_payment_update']
                    ?? ''
                )
            ),
            'karandash_descr' => trim(
