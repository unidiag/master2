...
if ($module === 'reports') {
    $distributors = isset($config['digital']['distributors'])
        && is_array($config['digital']['distributors'])
    ...
    foreach ($distributors as $distributor) {
        ...
        $reportDistributors[] = $distributor;
    }
        ...
        try {
            ...
            echo json_encode(
                [
                    ...
                    'reader' => $reader,
                    'enabled' => $enabled ? 1 : 0,
                    'running' => $readerService->isRunning(
                        $reader
                    ),
                ],
                JSON_UNESCAPED_UNICODE
...
if (
    ...
    /*
     * Активные отключения для отображаемых
     * заявок / подключений.
     */
    ...
    foreach ($data['rows'] ?? [] as $row) {
        ...
        if ($module === 'zayavki') {
            ...
            /*
             * В старых заявках адрес может
             * находиться в address_ajax.
             */
            if ($address === '') {
                $address = trim(
                    (string) (
    ...
    /*
     * Добавляем disconnect непосредственно
     * внутрь каждой строки $data['rows'].
     */
    if (
        isset($data['rows'])
        && is_array($data['rows'])
                ...
                if ($separatorPosition !== false) {
                    ...
                    if (
                        ...
                        foreach (
                            ...
                            $houseApartmentNumberRaw = trim(
                                (string) (
                                    $houseApartment['number']
                                    ?? ''
                                )
                            );
                        ...
                        foreach (
                            ...
                            $nearbyNumberRaw = trim(
                                (string) (
                                    $nearbyApartment['number']
                                    ?? ''
                                )
                            );
                            ...
                            */
                            ...
                            /*
                            * Компенсация у начала дома.
                            *
                            * Например:
                            * квартира 1 -> 1..21
                            * текущую квартиру ниже исключаем,
                            * значит фактически проверяем 2..21.
                            */
                            if ($rangeStart < 1) {
                                $rangeEnd +=
                                    1 - $rangeStart;
                            ...
                            /*
                            * Компенсация у конца дома.
                            *
                            * Например:
                            * квартира 100 при 100 квартирах
                            * -> 80..100
                            * текущую квартиру исключаем,
                            * значит фактически проверяем 80..99.
                            */
                            if (
                                $maxApartmentNumber > 0
                                && $rangeEnd > $maxApartmentNumber
                            ) {
                                $rangeStart -=
                                    $rangeEnd
                                    - $maxApartmentNumber;
                            ...
                            if (
                                $nearbyNumber < $rangeStart
                                || $nearbyNumber > $rangeEnd
                            ) {
                                continue;
                            }

                            $nearbyDebt = (float) (
                                $nearbyApartment['debt']
                                ?? 0
                            );
...
if ($module === 'stat') {
    ...
    if ($selectedPersonal !== '') {
        ...
        */
        ...
        /*
        * Добавляем в историю заявки
        * и подключения этого абонента.
        */
        if ($subscriberAddress !== '') {
            ...
            * Заявки.
            *
            * Используем существующий TicketRepository,
            * чтобы не дублировать SQL.
            */
            $ticketData = $tickets->list(
                $subscriberAddress,
                'all',
                1000,
                0
            ...
            foreach (
                ...
                /*
                * Поиск Repository может искать
                * не только по адресу, поэтому
                * обязательно проверяем точное
                * совпадение адреса.
                */
                if (
                    $ticketAddress
            ...
            * Подключения.
            */
            $connectionData = $connections->list(
                $subscriberAddress,
                'all',
                1000,
                0
            ...
            foreach (
                ...
                $connectionTimestamp =
                    strtotime($connectionTime);

                if (
                    $connectionTimestamp === false
                ) {
                    continue;
                }
            ...
            /*
            * До этого массив уже сортировался
            * после добавления отключений.
            *
            * Но мы только что добавили ещё
            * заявки и подключения, поэтому
            * сортируем всю ленту повторно.
            */
            usort(
                $payments,
                static function (
...
if ($module === 'otkluchki') {
    ...
    if ($search !== '') {
        $where[] = '
            (
                ...
                OR subscriber LIKE :search_subscriber
                OR personal LIKE :search_personal
                OR phone LIKE :search_phone
                OR tariff LIKE :search_tariff
            )
        ';
    ...
    $stmt = $pdo->prepare(
        ...
        SELECT
            ...
            (
                SELECT d.tarif
                FROM master_database d
                WHERE d.personal = o.personal
                ORDER BY d.id DESC
                LIMIT 1
            ) AS current_tariff
