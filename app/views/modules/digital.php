    ...
    $digitalDistributors = array_values(
        array_filter(
            array_map(
                static function ($distributor): array {
            ...
            static fn (array $distributor): bool =>
                $distributor !== []
        )
    );
        ...
        <?php if (
            ...
            <div class="digital-filter-bar">
                ...
                >
                    ...
                    <?php foreach ($digitalDistributors as $distributor): ?>
                        <option
                            value="<?= e($distributor['name']) ?>"
                        >
                            <?= e($distributor['name']) ?>
                        </option>
                    <?php endforeach; ?>
...
document.addEventListener(
    ...
    function () {
        ...
        const storageKey = 'master2.digital.filters';

        function saveDigitalFilters() {
            const filters = {
                server: serverSelect
                    ? serverSelect.value
                    : '',
                distrib: distribSelect
                    ? distribSelect.value
                    : '',
                expensive: expensiveCheckbox
        ...
        function loadDigitalFilters() {
            ...
            if (
                serverSelect
                && typeof filters.server === 'string'
            ) {
                serverSelect.value = filters.server;
            }

            if (
                distribSelect
                && typeof filters.distrib === 'string'
            ) {
                distribSelect.value = filters.distrib;
            }

            if (
                expensiveCheckbox
                && typeof filters.expensive === 'boolean'
            ) {
                expensiveCheckbox.checked =
                    filters.expensive;
            }
        ...
        function onDigitalFilterChange() {
            saveDigitalFilters();
