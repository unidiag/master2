...
document.querySelectorAll('[data-complete]').forEach((button) => {
  button.addEventListener('click', () => {
    ...
    const reconnectField =
        document.getElementById(
            'complete-reconnect-field'
        );

    const reconnectCheckbox =
        document.getElementById(
            'complete-reconnect'
        );
    ...
    const disconnectAddress =
        document.getElementById(
            'complete-disconnect-address'
        );

    const subscriberDisconnected =
        document.getElementById(
            'complete-subscriber-disconnected'
        );

    if (disconnectAddress) {
        disconnectAddress.value =
            data.address || '';
    }

    if (subscriberDisconnected) {
        subscriberDisconnected.value =
    ...
    if (
        ...
        if (data.disconnected) {
            reconnectField.hidden = false;
            reconnectCheckbox.checked = true;
        } else {
            reconnectField.hidden = true;
...
if (ticketCreateForm instanceof HTMLFormElement) {
  ...
  const debtLabelElement = ticketCreateForm.querySelector(
    '[data-subscriber-debt-label]'
  );
  ...
  const clearSubscriber = () => {
    ...
    if (debtLabelElement instanceof HTMLElement) {
      debtLabelElement.textContent = 'Долг';
    }
  ...
  const renderSubscriber = (
    ...
    if (debtElement instanceof HTMLElement) {
      const debt = Number(subscriber.balance || 0);

      if (debtLabelElement instanceof HTMLElement) {
        debtLabelElement.textContent =
          debt < 0 ? 'Аванс' : 'Долг';
      }
...
document.addEventListener(
    ...
    function (event) {
        ...
        /*
         * Убираем плюсы в начале,
         * затем добавляем ровно один.
         */
        value = value.replace(/^\++/, '');
